...
export default class OrderShow {
    ...
    populateForm(response) {
        ...
        const [
            ...
            province,
            city
        ] = this.data.shipping_address.split(', ');
    ...
    bindEvents() {
        $(document).on('click', '#view-receipt', () => {
            ...
            if (this.data.payment_method)
                $('#rcpt_order_method').text(this.data.payment_method);
            else
                $('#rcpt_order_method').text('Cash on Delivery');

            if (this.data.status == 'completed')
                $('#rcpt_order_paiddate').text(this.data.updated_at.split('T')[0]);
            else
    ...
    bindActions() {
        ...
        Object.keys(buttonStatusMap).forEach((button) => {
            $(document).on('click', button, () => {
                ...
                this.confirmAction[statusKey](() => { this.updateStatus(statusKey) });
            });
        });
    }
    init() {
